...
app.use(
  session({
    ...
    store: MongoStore.create({
      mongoUrl: "mongodb://127.0.0.1:27017/evaluatePrac",
    }),
    cookie: {
...
const scheduleJob = async () => {
  try {
    ...
    for (const teacher of allTeachers) {
      ...
      console.log(
        `Fetched evaluations for teacher ${teacher._id}:`,
        evaluations.length
      );
      ...
      evaluations.forEach((evaluation) => {
        ...
        evaluationsByTeacherAndSubject[teacherId][subject].evaluations.push(
          evaluation
        );
      });
    }
    ...
    for (const teacherId in evaluationsByTeacherAndSubject) {
      for (const subject in evaluationsByTeacherAndSubject[teacherId]) {
        const { evaluations, categoryComputation, department } =
          evaluationsByTeacherAndSubject[teacherId][subject];
        ...
        for (const category of categories) {
          ...
          console.log(
            `Category ${category}: Found ${questions.length} questions.`
          );
        }
        ...
        evaluations.forEach((evaluation) => {
          ...
          if (categoryComputation[category]) {
            categoryComputation[category].uniqueStudents.add(
              evaluation.IDstudent.toString()
            );
        ...
        for (const category of categories) {
          ...
          const numberOfStudents =
            categoryComputation[category].uniqueStudents.size;
          ...
          console.log(
            `Teacher: ${teacherId}, Subject: ${subject}, Category: ${category}, Metric: ${metric}, Unique Students: ${numberOfStudents}`
          );
          console.log(
            `Teacher: ${teacherId}, Subject: ${subject}, Category: ${category}, Total Sum: ${totalSum}, Number of Questions: ${numberOfQuestions}`
          );
        }

        const grandScore = parseFloat(
          (
            categoryScores.reduce((acc, score) => acc + score, 0) /
            categories.length
          ).toFixed(2)
        ...
        const calculatedEvaluation = new CalculatedEvaluation({
          ...
          category6Score:
            perCategoryMetrics["learning experiences and interaction"],
        ...
        console.log(
          `Calculated evaluation saved for Teacher: ${teacherId}, Subject: ${subject}, Grand Score: ${grandScore}`
        );
      }
    }
...
app.post("/saveStudents", async (req, res) => {
  for (let i = 0; i < studentId.length; i++) {
    ...
    const addAcc = new Student({
      ...
      name: name,
      program: studentProgram[i],
      yearLevel: studentYearLevel[i],
    });
    addAcc.save();
  }
...
app.post("/process-excel", upload.single("excelFile"), async (req, res) => {
  try {
    ...
    const studentEnrolled = Object.keys(sheet5)
      .filter(
        (cell) => cell.startsWith("F") && sheet[cell].v !== "enrolledSubs"
      ) // assuming header row
    ...
    console.log({
      success: true,
      message: "Student infosc extracted successfully",
    });
    ...
    res
      .status(500)
      .json({ success: false, message: "Error processing Excel file" });
  }
});
...
app.get("/facInstructors", authMiddleware, async (req, res) => {
  ...
  res.json({
    ...
    textedQuestions: textedQuestions,
...
app.get(
  "/:guidanceID/getCalculatedEvaluation",
  authMiddleware,
  guidanceMiddleware,
  async (req, res) => {
    try {
      ...
      const subjectIds = foundCalculatedEvaluations.map(
        (subject) => subject.subjectId
      );

      const subjectCodesArray = await Promise.all(
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      if (
        !foundCalculatedEvaluations ||
        foundCalculatedEvaluations.length === 0
      ) {
        return res
          .status(404)
          .json({ message: "No teachers found for the specified criteria" });
      }

      // Fetch teacher details from the instructors collection based on unique teacher IDs
      const teachers = await Instructor.find({
        _id: { $in: foundCalculatedEvaluations },
      }).populate("subjects");
...
app.post(
  "/:studentID/instructors/:teacherID/:subjectcode/evaluate",
  authMiddleware,
  studentMiddleware,
  async (req, res) => {
    const answers = req.body;
...
app.get(
  ...
  async (req, res) => {
    ...
    const category1Question = await Question.find({
      questionCategory: "course overview and orientation",
    });
    const category2Question = await Question.find({
      questionCategory: "learning outcomes",
    });
    const category3Question = await Question.find({
      questionCategory: "assessment",
    });
    const category4Question = await Question.find({
      questionCategory: "instructional material/content",
    });
    const category5Question = await Question.find({
      questionCategory: "teacher presence and support",
    });
    const category6Question = await Question.find({
      questionCategory: "learning experiences and interaction",
    });
    const category7Question = await Question.find({
      questionCategory: "technology and accessibility",
    });

    res.render("mobile1stQuestion", {
      category1Question,
      category2Question,
      category3Question,
      category4Question,
  ...
  async (req, res) => {
    ...
    try {
      const foundStudent = await Student.findOne({
        studentID: studentID,
      }).populate("subjectsEnrolled evaluatedSubjects");
      const studentSubjects = foundStudent.subjectsEnrolled;
      const evaluatedSubjects = foundStudent.evaluatedSubjects.map((subject) =>
      ...
      res.render("mobileSelectInstructors", {
        teacherSubjectPairs,
        studentId: studentID,
      });
    } catch (error) {
...
app.post(
  "/:adminid/cancelTimeFrame",
  authMiddleware,
  adminMiddleware,
  async (req, res) => {
    try {
      // Fetch the latest time frame
      const latestTimeFrame = await TimeFrame.findOne({ isActive: true }).sort({
        _id: -1,
      });
      ...
      if (!latestTimeFrame.isActive) {
        return res
          .status(400)
          .json({ message: "Time frame is already inactive" });
      }
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const endDate = moment(doc.endDate)
        .tz("Asia/Manila")
        .format("YYYY-MM-DD");
      ...
      console.log(
        "Calculated End DateTime (Philippines Time):",
...
app.put(
  "/:adminId/:questionId/updateQuestion",
  authMiddleware,
  adminMiddleware,
  async (req, res) => {
...
app.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ message: "Failed to delete students", error: error.message });
    }
  }
);

app.post(
  "/:adminId/addBulkTeacher",
  authMiddleware,
  adminMiddleware,
  async (req, res) => {
...
app.delete(
  "/:adminId/deleteIndividualSubject",
  authMiddleware,
  adminMiddleware,
  async (req, res) => {
    try {
      ...
      if (!foundSubject) {
        res
          .status(404)
          .json({ message: "error finding subject:", subjectCode });
      }
      ...
      res
        .status(200)
        .json({ message: "Successful deleting subject:", subjectCode });
    } catch (error) {
...
app.get(
  "/:adminId/getIndividualStudentSubject",
  authMiddleware,
  adminMiddleware,
  async (req, res) => {
    const { department } = req.query;
...
app.get(
  "/:adminId/getConfirmDeleteStudent",
  authMiddleware,
  adminMiddleware,
  async (req, res) => {
    try {
...
app.delete(
  "/:adminId/deleteIndividualStudent",
  authMiddleware,
  adminMiddleware,
  async (req, res) => {
    const { adminId } = req.params;
    ...
    try {
      ...
      console.log(
        "there is an error deleting individual student",
        error.message
      );
    }
  }
...
app.put(
  "/:adminId/:studentId/updateIndividualStudent",
  authMiddleware,
  adminMiddleware,
  async (req, res) => {
    ...
    try {
      ...
      console.log(
        "There is an error creating individual student:",
        error.message
      );
  ...
  async (req, res) => {
    try {
      ...
      const existingStudent = await Student.findOne({
        $or: [{ studentID: studentID }, { email: studentEmail }],
      });
    ...
    try {
      for (const row of rows) {
        const {
          ...
          year_level,
          program,
          enrolledSubs: subjectsString,
          email,
        } = row;

        // Check if the student already exists
        const existingStudent = await Student.findOne({
          $and: [{ studentID: studentId }, { email: email }],
        });
...
app.get(
  "/:admin/getFilteredSubjects",
  authMiddleware,
  adminMiddleware,
  async (req, res) => {
    try {
      const { subjectDep, subjectYear, subjectProgram } = req.query;
      const foundSubs = await Subject.find({
        department: subjectDep,
        year: subjectYear,
        program: subjectProgram,
      });
...
app.get(
  "/:adminId/cetDepartmentInstructors",
  authMiddleware,
  adminMiddleware,
  async (req, res) => {
    const allTeachers = await Instructor.find({ department: "CET" }).populate(
      "subjects"
    );
...
app.get(
  "/:adminId/cnhsDepartmentInstructors",
  authMiddleware,
  adminMiddleware,
  async (req, res) => {
    const allTeachers = await Instructor.find({ department: "CNHS" }).populate(
      "subjects"
    );
    res.json({ allTeachers: allTeachers });
  }
...
app.get(
  "/:adminId/chmDepartmentInstructors",
  authMiddleware,
  adminMiddleware,
  async (req, res) => {
    const allTeachers = await Instructor.find({ department: "CHM" }).populate(
      "subjects"
    );
    res.json({ allTeachers: allTeachers });
  }
...
app.get(
  "/:adminId/cbaDepartmentInstructors",
  authMiddleware,
  adminMiddleware,
  async (req, res) => {
    const allTeachers = await Instructor.find({ department: "CBA" }).populate(
      "subjects"
    );
    res.json({ allTeachers: allTeachers });
  }
...
app.get(
  "/:adminId/ceaaDepartmentInstructors",
  authMiddleware,
  adminMiddleware,
  async (req, res) => {
    const allTeachers = await Instructor.find({ department: "CEAA" }).populate(
      "subjects"
    );
    res.json({ allTeachers: allTeachers });
  }
...
app.get(
  "/:adminId/ccjeDepartmentInstructors",
  authMiddleware,
  adminMiddleware,
  async (req, res) => {
    const allTeachers = await Instructor.find({ department: "CCJE" }).populate(
      "subjects"
    );
    res.json({ allTeachers: allTeachers });
  }
...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (userType === "student") {
      ...
      const timeFrame = await TimeFrame.findOne({ isActive: true }).sort({
        _id: -1,
      });
      ...
      if (timeFrame) {
        ...
        const [startHour, startMinute] = timeFrame.startTime
          .split(":")
          .map(Number);
        const [endHour, endMinute] = timeFrame.endTime.split(":").map(Number);
    ...
    if (
      (userType === "admin" || userType === "guidance") &&
      !foundUser.hasChangedPassword
    ) {
      return res.redirect(`/${foundUser._id}/setpassword`);
    }
